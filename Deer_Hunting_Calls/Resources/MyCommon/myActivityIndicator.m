...
{	
	if ([self superview] != [[UIApplication sharedApplication] keyWindow]) 
    {
        ...
        if (viewToBlockTouch != nil)
            viewToBlockTouch = nil;
        
        viewToBlockTouch = [[UIView alloc] initWithFrame:keyWindow.frame];
        [[[UIApplication sharedApplication] keyWindow] addSubview:viewToBlockTouch];
...
- (void)hidden
{
    ...
    [viewToBlockTouch removeFromSuperview];
